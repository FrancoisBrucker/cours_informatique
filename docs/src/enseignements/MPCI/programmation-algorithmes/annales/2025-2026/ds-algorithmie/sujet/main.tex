...
\item sont chacun constitués de 3 parties de difficultés croissantes.
\end{itemize}
...
\begin{exo}[Boyer-Moore-Horspool]
    ...
    \begin{enumerate}
        ...
        \item pour un indice $i$ donné, on fait décroître un indice $j$ de $m-1$ à $0$.
        \begin{itemize}
            \item si $a[i + j] = b[j]$ pour tout $j$, l'algorithme s'arrête et rend $i$
            \item sinon, lors de la première non correspondance ($a[i + j] \neq b[j]$ et $a[i + j'] = b[j']$ pour $j < j' < m$), l'indice $i$ est {\bf décalé} :
            \begin{itemize}
    ...
    \begin{partie}[Recherche]
        \begin{questionpartie}
            Écrivez le pseudo-code de l'algorithme de Boyer-Moore-Horspool. Il devra utiliser la fonction \texttt{décalage(b: chaîne) $\rightarrow$ [entier]} de la partie précédente.
        \end{questionpartie}
        \begin{questionpartie}
            Quelle est la complexité maximale et minimale de votre algorithme de Boyer-Moore-Horspool ?
        \end{questionpartie}
        \begin{questionpartie}
...
Par exemple on a $[0, 0, 0, 1, 2, 3, 0] = \text{décalage}("ABABACA")$ :~\\
    ...
    \begin{partie}[Preuve de l'algorithme]
