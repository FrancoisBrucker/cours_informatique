...
\begin{exo}[Itératif]
    ...
    \begin{partie}[Indice $i^\star_{T}$]
        ...
        \end{questionpartie}
        \begin{questionpartie}
            Écrivez un algorithme de complexité linéaire (vous le justifierez) qui prend en entrée un élément $T$ de $\mathcal{T}_n$ et rend $i^\star_{T}$. Il sera de signature~:
    ...
    \begin{partie}[Successeur]
        ...
        \end{questionpartie}
        \begin{questionpartie}
            Écrivez un algorithme de signature \verb|suivant(T: [entier]) → |$\emptyset$  qui prend en entrée un élément $T$ de $\mathcal{T}_n$ et {\bf le modifie} en son successeur (pour l'ordre lexicographique dans $\mathcal{T}_n$). Sa complexité devra être linéaire (vous le justifierez).
        \end{questionpartie}
        \begin{questionpartie}
...
\begin{exo}[Optimal]
    ...
    \begin{partie}[Propriétés]
        Soit $T$ un tableau. On va étudier ses modifications après exécution de l'algorithme. Pour cela, on note $T'$ le tableau $T$ après l'exécution de $\textsc{heap}(T, k)$.
            \begin{questionpartie}
    ...
    \begin{partie}[Optimalité]
        \begin{questionpartie}
            Démontrez que lors de l'exécution de $\textsc{heap}(T, k)$, il y a eu exactement $k!-1$ échanges. En déduire la complexité de l'affichage à l'écran de tous les éléments de $\mathcal{T}_n$.
        \end{questionpartie}
        \begin{questionpartie}
            Quelle est la complexité spatiale de l'algorithme $\textsc{heap}$ ?
        \end{questionpartie}
