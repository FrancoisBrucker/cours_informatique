...
```javascript
> contenu_du_fichier = fs.readFileSync("./numérologie.js")
...
```javascript
> contenu_du_fichier = fs.readFileSync("./numérologie.js", {encoding:'utf8'})
[... snip ...]
>
```
...
* créez un nouveau dossier `numérologie/tests/`{.fichier}
* créez un fichier `numérologie/tests/tests_numérologie.js`{.fichier} où l'on pourra recopier nos tests.

Le fichier `numérologie/tests/tests_numérologie.js`{.fichier} :
...
On peut maintenant supprimer vos tests de *"numérologie/numérologie.js*. On pourra exécuter nos tests facilement dans terminal en se plaçant dans le dossier *"numérologie/tests"* puis en exécutant la commande : `node test_numérologie.js`{.fichier}.
...
* on importe depuis un fichier dans le dossier `tests`{.fichier}. Le fichier `numérologie/numérologie.js`{.fichier} étant dans le dossier parent, on utilise la [notation *"../*"]({{"/tutoriels/fichiers-navigation/" | url}}#block-.-..).
