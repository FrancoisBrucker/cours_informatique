...
- le _protocole_ est `https`
- le _serveur_ est `fr.wikipedia.org`
- la _ressource_ est `wiki/Uniform_Resource_Locator`
...
- une adresse IP comme `[2a02:ec80:600:ed1a::1]` (on encadre l'adresse par des `[]` lorsque l'on utilise des adresse ipv6, on ne le fait pas pour des adresse ipv4)
...
{% note %}
Il n'est pas nécessaire d'être accordé au réseau pour résoudre l'adresse `localhost` et `[::1]`. Elles sont souvent utilisées lorsque l'on crée ses propres serveur web.
{% endnote %}
{% info %}
Vous verrez parfois l'adresse `127.0.0.1` pour désigner localhost. C'est une adresse IPv4 qui doit être considéré comme obsolète : ne l'utilisez plus.
{% endinfo %}
...
Chaque protocole a cependant un port par défaut qui est utilisé s'il aucun port n'est précisé. Par exemple, le port par défaut du protocole https est le 443 et donc les urls `https://www.google.com` et `https://www.google.com:443` sont identiques.

{% faire %}
Essayez d'atteindre avec chrome les 2 urls : `https://www.google.con` et `https://www.google.com:443`. Que donne l'url `https://www.google.com:8080` ? A priori elle ne fonctionne pas car la roue tourne mais ne s'arrête pas.
{% endfaire %}
