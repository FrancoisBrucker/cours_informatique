...
Comme $a \bmod m$ est égal au reste de la division entière de $a$ par $m$ son calcul est bien déterministe.
...
- $7 \bmod 3 = (4 \bmod 3) + (3 \bmod 3) = 1 + 0 = 1$
- $4 \times 3 \bmod 3 = (4 \bmod 3) \times (3 \bmod 3) = 1 \times 0 = 0$
...
$$
\begin{array}{lcl}
n \bmod m &=&  (\sum_{i=0}^l n_i2^{ki}) \bmod m \\
&=& (\sum_{i=0}^l ((n_i \bmod m)\cdot ((2^{k} \bmod m)^i \bmod m) \bmod m)) \bmod m
\end{array}
$$
</div>
...
Attention cependant, les nombres qui ont un diviseur commun avec $m$ seront hachés par un nombre qui est un multiple de ce diviseur car $(k \times p) \bmod (p \times q) = (k \bmod q) \times p$.
