...
{% info %}
On a utilisé une technique différente des listes pour nos `struct`{.language-}. On n'y expose que `element`{.language-} qui sont des pointeurs et non directement la structure interne.
{% endinfo %}
